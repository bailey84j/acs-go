...
type clustercveTests struct {
	Name   string
	Args   map[string]interface{}
	Result string
}

func TestSuppressCVEs(t *testing.T) {
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.ClusterCVE.SuppressCVEs(testParams.args)
        			if msi != nil {
...
func TestUnsuppressCVEs(t *testing.T) {
	...
	tableFile := "../../test_tables/clustercve/unsuppresscves.json"
	State, err := ioutil.ReadFile(tableFile)
	if err != nil {
		t.Errorf("could not read test table file: %d", err)
	}
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.ClusterCVE.UnsuppressCVEs(testParams.args)
        			if msi != nil {
				jsonbyte, err := json.Marshal(msi)
				if err != nil {
					t.Errorf("error parsing JSON string - %s", err)
