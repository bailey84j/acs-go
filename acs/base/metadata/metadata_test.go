...
func TestGetMetadata(t *testing.T) {
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.Metadata.GetMetadata(testParams.args)
        			if msi != nil {
...
func TestTLSChallenge(t *testing.T) {
	...
	tableFile := "../../test_tables/metadata/tlschallenge.json"
	State, err := ioutil.ReadFile(tableFile)
	if err != nil {
		t.Errorf("could not read test table file: %d", err)
	}
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.Metadata.TLSChallenge(testParams.args)
        			if msi != nil {
				jsonbyte, err := json.Marshal(msi)
				if err != nil {
					t.Errorf("error parsing JSON string - %s", err)
