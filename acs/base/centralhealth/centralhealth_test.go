...
type centralhealthTests struct {
	Name   string
...
func TestGetUpgradeStatus(t *testing.T) {
	...
	tableFile := "../../test_tables/centralhealth/getupgradestatus.json"
	State, err := ioutil.ReadFile(tableFile)
	if err != nil {
		t.Errorf("could not read test table file: %d", err)
	}

	var getTests []centralhealthTests
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.CentralHealth.GetUpgradeStatus(testParams.args)
        			if msi != nil {
