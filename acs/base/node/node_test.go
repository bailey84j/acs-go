...
func TestListNodes(t *testing.T) {
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.Node.ListNodes(testParams.clusterId,testParams.args)
        			if msi != nil {
...
func TestGetNode(t *testing.T) {
	...
	tableFile := "../../test_tables/node/getnode.json"
	State, err := ioutil.ReadFile(tableFile)
	if err != nil {
		t.Errorf("could not read test table file: %d", err)
	}
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			type params struct {
				fieldTypes string                 `json:"fieldTypes"`
				args       map[string]interface{} `json:"args"`
                clusterId string `json:"clusterId"`nodeId string `json:"nodeId"`			
            }
			...
			msi, err := acs.Node.GetNode(testParams.clusterId,testParams.nodeId,testParams.args)
        			if msi != nil {
				jsonbyte, err := json.Marshal(msi)
				if err != nil {
					t.Errorf("error parsing JSON string - %s", err)
