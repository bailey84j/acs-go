...
type probeuploadTests struct {
	Name   string
	Args   map[string]interface{}
	Result string
}

func TestGetExistingProbes(t *testing.T) {
	...
	tableFile := "../../test_tables/probeupload/getexistingprobes.json"
	State, err := ioutil.ReadFile(tableFile)
	if err != nil {
		t.Errorf("could not read test table file: %d", err)
	}

	var getTests []probeuploadTests
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.ProbeUpload.GetExistingProbes(testParams.args)
        			if msi != nil {
