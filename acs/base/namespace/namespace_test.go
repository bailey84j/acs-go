...
func TestGetNamespaces(t *testing.T) {
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.Namespace.GetNamespaces(testParams.args)
        			if msi != nil {
...
func TestGetNamespace(t *testing.T) {
	...
	tableFile := "../../test_tables/namespace/getnamespace.json"
	State, err := ioutil.ReadFile(tableFile)
	if err != nil {
		t.Errorf("could not read test table file: %d", err)
	}
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.Namespace.GetNamespace(testParams.id,testParams.args)
        			if msi != nil {
				jsonbyte, err := json.Marshal(msi)
				if err != nil {
					t.Errorf("error parsing JSON string - %s", err)
