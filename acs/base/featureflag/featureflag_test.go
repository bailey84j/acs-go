...
func TestGetFeatureFlags(t *testing.T) {
	...
	tableFile := "../../test_tables/featureflag/getfeatureflags.json"
	State, err := ioutil.ReadFile(tableFile)
	if err != nil {
		t.Errorf("could not read test table file: %d", err)
	}

	var getTests []featureflagTests
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.FeatureFlag.GetFeatureFlags(testParams.args)
        			if msi != nil {
