...
func TestGetTelemetryConfiguration(t *testing.T) {
	...
	tableFile := "../../test_tables/telemetry/gettelemetryconfiguration.json"
	State, err := ioutil.ReadFile(tableFile)
	if err != nil {
		t.Errorf("could not read test table file: %d", err)
	}

	var getTests []telemetryTests
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.Telemetry.GetTelemetryConfiguration(testParams.args)
        			if msi != nil {
...
func TestConfigureTelemetry(t *testing.T) {
	...
	tableFile := "../../test_tables/telemetry/configuretelemetry.json"
	State, err := ioutil.ReadFile(tableFile)
	if err != nil {
		t.Errorf("could not read test table file: %d", err)
	}
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.Telemetry.ConfigureTelemetry(testParams.args)
        			if msi != nil {
				jsonbyte, err := json.Marshal(msi)
				if err != nil {
					t.Errorf("error parsing JSON string - %s", err)
