...
type credentialexpiryTests struct {
	Name   string
...
func TestGetCertExpiry(t *testing.T) {
	...
	tableFile := "../../test_tables/credentialexpiry/getcertexpiry.json"
	State, err := ioutil.ReadFile(tableFile)
	if err != nil {
		t.Errorf("could not read test table file: %d", err)
	}

	var getTests []credentialexpiryTests
	...
	for _, tt := range getTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			msi, err := acs.CredentialExpiry.GetCertExpiry(testParams.args)
        			if msi != nil {
